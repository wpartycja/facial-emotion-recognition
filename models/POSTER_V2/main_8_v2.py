...
def main():
    ...
    # if args.predict:
    #     for i, file in enumerate(os.listdir(valdir)):
    #         image = cv2.imread(f'{valdir}/{file}')

    #         image = np.expand_dims(image, axis=0)
    #         print("First image size: ", image.shape)

    #         image = np.moveaxis(image, 3, 1)
    #         print("Size after move axis: ", image.shape)

    #         # image_res = cv2.resize(image, dsize=(1, 224, 224))
    #         # print("after resize:", image_res.shape)

    #         image_tensor = torch.Tensor(image)
    #         print("tensor size: ", image_tensor.shape)

    #         x_face = F.interpolate(image_tensor, size=(112))
    #         print("after interpolate: ", x_face.shape)

    #         with torch.no_grad():
    #             image_cuda = x_face.cuda()
    #             output = model(image_cuda)
    #             print(f'Face {i}: emotion: {output}')
